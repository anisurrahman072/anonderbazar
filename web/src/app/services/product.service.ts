...
export class ProductService {
  ...
  filter_result(searchTerm:String, typeList:number[],categoryList: number[],warehouses: number[],craftsmanList: number[], subcategoryList: number[], brandList: number[], subsubcategoryList: number[], priceRange: number[], sortTitle: string, sortTerm: String,pageno:number): Observable<any> {
    ...
    return this.http.get(`${this.EndPoint2}/search?filters={"searchTerm":"${searchTerm}","typeList":[${categoryList}],"categoryList":[${subcategoryList}],"brandList":[${brandList}],"warehousesList":[${warehouses}],"subcategoryList":[${subsubcategoryList}],"craftsmanList":[${craftsmanList}],"priceRange":[${priceRange}]}&sortTitle=${sortTitle}&sortTerm=${sortTerm}&limit=500&page=${pageno}`);
  }
  ...
  getAllPromotionProduct(): Observable<any> {
    ...
    return this.http
      .get(this.EndPoint + '?where={"deletedAt":null,"promotion":1,"end_date":{">=":"'+date+'"}}')
      .map(response => response);
  }
  getAllRatingProduct(): Observable<any> {
        return this.http
            .get(this.EndPoint + '?where={"deletedAt":null}&limit=3&sort="rating%20DESC"')
            .map(response => response);
  }
  ...
  getAllByWarehouseId(id): Observable<any> {
    return this.http
      .get(`${this.EndPoint}?where={"deletedAt":null,"warehouse_id":${id}}`)
      .map(response => response);
  }

  getAllByCategoryId(id): Observable<any> {
    return this.http
      .get(`${this.EndPoint}?where={"deletedAt":null,"category_id":${id}}`)
      .map(response => response);
  }  

  getAllFeatureProducts(): Observable<any> {
    return this.http
      .get(this.EndPoint + '?where={"deletedAt":null, "featured":1}')
      .map(response => response);
  }
  getFlashDealsProducts(): Observable<any> {
    return this.http
      .get(this.EndPoint + '?where={"deletedAt":null, "featured":1}&limit=4')
      .map(response => response);
  }

  getRewardProducts(): Observable<any> {
    return this.http
      .get(this.EndPoint + '?where={"deletedAt":null}&limit=4')
      .map(response => response);
  }
  ...
  getFeedbackProducts(): Observable<any> {
    return this.http
      .get(this.EndPoint + '?where={"deletedAt":null}&limit=3&sort="rating DESC"')
      .map(response => response);
  }

  getNewProducts(): Observable<any> {
    return this.http
      .get(this.EndPoint + '?where={"deletedAt":null,"featured":0}&limit=3&sort="createdAt DESC"')
      .map(response => response);
  }

  getRecommendedProducts(limit, offset): Observable<any> {
    return this.http
      .get(`${this.EndPoint}?where={"deletedAt":null}&limit=${limit}&skip=${offset}`)
      .map(response => response);
  }
  ...
  getByCategory(catId: number, productID:number): Observable<any> {
    return this.http
      .get(`${this.EndPoint}?where={"deletedAt":null,"id":{"!":${productID}},"category_id":${catId}}`)
      .map(response => response);
  }
