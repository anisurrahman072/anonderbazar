...
export class CmsService {
  ...
  getBySubSectionName(pageName: String,sectionName: String, subSectionName: any): Observable<any> {
    return this.http
      .get(
        this.EndPoint +
          `?where={"page":"${pageName}","section":"${sectionName}","sub_section":"${subSectionName}","deletedAt":null}`
      )
      .map(response => response);
  }
  ...
  getRecentPost(pageName: String,limit: number, column: String,orderby:String ): Observable<any> {
    return this.http
      .get(
        this.EndPoint +
          // `?where={"page":"${pageName}","deletedAt":null}&"limit=${limit}&sort=${column} ${orderby}`
          `?where={"page":"${pageName}","deletedAt":null}&limit=${limit}&sort=${column}%20${orderby}`
      )
      .map(response => response);
  }
}
