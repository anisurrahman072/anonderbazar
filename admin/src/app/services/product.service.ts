...
export class ProductService {
    ...
    getAllWithPagination(page = 1, limit = 30, excludedProductIds = [], nameSearchValue = '', codeSearchValue = '', shopSearchValue = '', brandSearchValue = '', categorySearchValue = '', subCategorySearchValue = '', productStatus?:number): Observable<any> {
        const skip = (page - 1) * limit;
        return this.http.get(this.EndPoint2 + `?where={"deletedAt":null}&codeSearchValue=${codeSearchValue}&nameSearchValue=${nameSearchValue}&shopSearchValue=${shopSearchValue}&brandSearchValue=${brandSearchValue}&categorySearchValue=${categorySearchValue}&subCategorySearchValue=${subCategorySearchValue}&productStatus=${productStatus}&excludedProductIds=${JSON.stringify(excludedProductIds)}&page=${page}&skip=${skip}&limit=${limit}`);
    }
