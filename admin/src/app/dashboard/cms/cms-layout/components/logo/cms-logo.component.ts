...
@Component({
  selector: 'app-cms-logo',
  templateUrl: './cms-logo.component.html',
  styleUrls: ['./cms-logo.component.css']
})
export class CmsLogoComponent implements OnInit {
  cmsLogoData: any;
  ...
  getData() {
    this.cmsService.getBySectionName('LOGO').subscribe(result => {
      this.id = result[0].id;
      this.cmsLogoData = result[0].data_value;
    });
  }
  //Method for showing the modal
  showEditModal = i => {
    this.currentLogoId = i;
    this.editValidateForm.patchValue(this.cmsLogoData[i]);
...
//Event method for submitting the edit form
  submitEditForm = ($event, value) => {
    ...
    formData.append('dataValueId', this.currentLogoId.toString());
