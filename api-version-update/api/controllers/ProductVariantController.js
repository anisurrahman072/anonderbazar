...
module.exports = {
  ...
  create: function (req, res) {
      ...
      if (req.body.variant_type == 0) {
          ...
          ProductVariant.create(body).exec(function (err, productVariant) {
              if (err) {
          ...
          ProductVariant.create(req.body).exec(function (err, productVariant) {
              if (err) {
                  return res.json(err.status, {err: err});
              }
              if (productVariant) {
  ...
  byvariant: function (req, res) {
      ...
      ProductVariant.find({product_id: req.param("product_id"), deletedAt: null})
          ...
          .exec(function (err, productVariants) {

              let variants = _.uniq(_.map(productVariants, "variant_id.id"));


              let data = [];
              async.each(variants, function (v, callback) {
